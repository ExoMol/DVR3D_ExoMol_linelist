program linelist_DVR3D
 ...
 integer j,jf,ji,ilevel,ilevelf,ileveli,igamma,igammaf,igammai,info,j0,i,k,jgamma,kminf,kmini,iparf,ipari
 ...
 integer(4)     :: gns(4) = (/1,1,0,0/)
 real(8),parameter :: e_thresh = 1d-3,tocm = 2.19474624d+05,todebye=2.5417662d0
 ...
 real(8),parameter :: planck=6.6260693d-27,avogno=6.0221415d+23,vellgt=2.99792458d+10,boltz=1.380658d-16,detosc=3.136186d-07
      ...
      interface
        subroutine get_energies_from_fort14(iunit,jrot,kpar,ipar,meval,lmax,jmax,ierr,enercutoff,enercalc)
    ...
    !
    allocate(sym(nlevels),energies(Lmax,0:jmax,4),Jktau(nlevels,3),NN(Lmax,0:jmax,4),stat=info); 
    ...
    do ifile = 1,Nenfiles
      !
       ...
       if (jmax>200) stop 'Jmax>200'
       !
       call get_energies_from_fort14(1,jrot,kpar,ipar,meval,Lmax,jmax,ierr,enercutoff,energies)
       !
       isym = rvsym(mod(jrot+2,2),kpar,ipar)
       iasym = rvsym(mod(jrot+2,2),mod(kpar+3,2),mod(ipar+3,2))
       !
      ...
      do isym = 1,4  
        ...
        do ilevel = 1,numj(jrot,isym)
          ...
          if (energeyfile_do) then
            ...
            !
            write(12,"(i12,1x,f12.6,1x,i6,1x,i7,1x,a4)") & 
               i,energies(ilevel,jrot,isym),gns(isym)*(2*jrot+1),jrot,symm(isym)
               !
          endif
        enddo
        !
    ...
    subroutine get_energies_from_fort14(iunit,jrot,kpar,ipar,meval,lmax,jmax,ierr,enercutoff,enercalc)
      ...
      integer(4)              :: i_t(7),isym,iasym,verbose=4
